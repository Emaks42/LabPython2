...
<li>В командах cp и mv предполагается явное указание нового имени директории/файла</li>
</ul>
...
или в кавычках или с экранированным пробелом)</li>
...
<li>Если встречаем обратный слеш, то изменяем зачение переменной screened на True (вернётся к False после следующего символа)</li>
...
Возвращет словарь, где опции соответствует bool.
...
<li>Крайне похожи между собой, по сути являются прямыми реализациями функций из библиотек</li>
...
## Тестирование
Для тестирования используется модуль pytest, как вспомогательный идёт pyfakefs, позволяющий создать виртуальную
...
Сама система имееет следующую структуру:
 ...
 <pre>
    ...
    │   ├── ls_folder/    # Папка, содержит файлы с числами от 0.txt до 2.txt
